      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="pt-24 pb-16">
          <div className="container mx-auto px-4">
            ...
            {/* Category Tabs */}
            <FadeInOnScroll delay={0.1} className="flex flex-wrap justify-center gap-3 mb-12">
              {skillCategories.map((category, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`flex items-center gap-2 px-5 py-3 font-bold uppercase text-sm tracking-wide border-4 border-foreground transition-colors ${
            ...
            <div className="max-w-5xl mx-auto">
              <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4" key={activeCategory}>
                  ...
                  <StaggerItem key={skill.name}>
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 border-4 border-foreground bg-accent flex items-center justify-center flex-shrink-0">
            ...
            <FadeInOnScroll className="max-w-5xl mx-auto mt-16">
              <h2 className="text-3xl font-bold mb-8 uppercase tracking-tight">All Categories</h2>
                ...
                {skillCategories.map((category, i) => {
                  ...
                  return (
                    <StaggerItem key={category.name}>
                      <motion.div 
                        className="border-4 border-foreground p-6 shadow-sm"
                        whileHover={{ y: -4, boxShadow: "var(--shadow-md)" }}
                        transition={{ duration: 0.2 }}
                      >
                        <div className="flex items-center gap-3 mb-4">
                          <div 
                            className="w-10 h-10 border-4 border-foreground flex items-center justify-center"
                            style={{ backgroundColor: `hsl(var(--${colors[i % 4]}))` }}
                          >
                            <Icon className="w-5 h-5" />
                          </div>
                          <div>
                            <h3 className="font-bold uppercase text-sm">{category.name}</h3>
                            <p className="text-sm text-muted-foreground font-mono">{category.skills.length} skills</p>
                          </div>
                        </div>
